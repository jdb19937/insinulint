/*
 * propositiones/spatium_virgula.c — propositio virgularum
 */

#include "spatium_virgula.h"
#include "../correctiones/virgula.h"

#include <string.h>

int propone_spatium_virgula(
    ...
    char *wp = corrige_virgulas(
        tmp, corpus + sp_init, corp_lon - sp_init
    );
    int clon = (int)(wp - tmp) - 1; /* sine '\n' */
